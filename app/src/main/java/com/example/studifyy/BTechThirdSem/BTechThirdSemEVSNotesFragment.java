...
public class BTechThirdSemEVSNotesFragment extends Fragment {
    RecyclerView recyclerView;
    ArrayList<LoadMaterialModal> list;
    ShimmerFrameLayout shimmerFrameLayout;

    public BTechThirdSemEVSNotesFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view= inflater.inflate(R.layout.fragment_b_tech_third_sem_e_v_s_notes, container, false);
        shimmerFrameLayout=view.findViewById(R.id.shimmer);
        shimmerFrameLayout.startShimmer();
        list=new ArrayList<>();
        recyclerView=view.findViewById(R.id.EVSNotesRecycleView);
        LoadMaterialRecycleViewAdapter adapter=new LoadMaterialRecycleViewAdapter(list,getActivity());
        recyclerView.setAdapter(adapter);
        recyclerView.setLayoutManager(new GridLayoutManager(getActivity(),3));
        FirebaseDatabase.getInstance().getReference().child("BTechThirdSemEVSNotes").addValueEventListener(new ValueEventListener() {
            @Override
